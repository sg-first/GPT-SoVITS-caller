...
p_train_GPT=None
def open1Bb(batch_size,total_epoch,exp_name,if_dpo,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers,pretrained_s1, GPT_weight_root):
    global p_train_GPT
    if(p_train_GPT==None):
...
def open_slice(inp,opt_root,threshold,min_length,min_interval,hop_size,max_sil_kept,_max,alpha,n_parts):
    ...
    if(os.path.exists(inp)==False):
        return "输入路径不存在",{"__type__":"update","visible":True},{"__type__":"update","visible":False}
    if os.path.isfile(inp):n_parts=1
    elif os.path.isdir(inp):pass
    else:
        return "输入路径存在但既不是文件也不是文件夹",{"__type__":"update","visible":True},{"__type__":"update","visible":False}
    if (ps_slice == []):
        ...
        for p in ps_slice:
            p.wait()
        ps_slice=[]
        return "切割结束",{"__type__":"update","visible":True},{"__type__":"update","visible":False}
